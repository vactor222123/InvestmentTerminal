...
Package 123 selects a schema-version-2 failure envelope with an allowlisted,
bounded, cycle-safe module/class chain and unchanged stable category behavior.
...
Package 145 records that batch 105 contains 19 successful outcomes and one
retryable `YahooCandleInvalidResponseError`, with no empty or final outcomes.
The halt is isolated to one private series. Run only the existing read-only
...
Package 156 records that the resumed sweep preserved 79,735 newly inserted
candles, completed sweep coverage through batch 108, and halted safely on a
new batch-109 `APIError`. Because the failure was written under checkpoint
...
Package 168 records the successful retry: the timeout cleared, 1,573 candles
were inserted, batch 576 now has zero blocking failures, and SQLite integrity
