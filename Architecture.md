...
The Phase 7 manifest boundary joins the checksum-bound private eligibility
...
`MarketBatchManifestService` implements this boundary. The private manifest and
its checksum remain separate from a privacy-safe
...
Private checkpoint schema 4 is implemented without weakening schema-3 terminal
isolation. New retryable failures preserve the
existing privacy-safe category and bounded exception-type chain at the atomic
checkpoint boundary. `ManifestPartialNoPriceIsolationService` is the separate
offline manifest-bound transition and may terminalize
...
remain separate and unchanged. Its CLI commits the private checkpoint before
its redacted report and has no provider, SQLite, importer, or sweep dependency.
# Phase 7 read-only batch checkpoint boundary

`ManifestBatchCheckpointDiagnostic` is an offline operations boundary over one
checksum-selected manifest request and its private checkpoint. It may validate
...
The implemented manifest- and immutable-inventory-bound partial timeout retry
handles the distinct batch-576 transient-retry requirement. A stored blocking
`TIMEOUT` cannot be retried by the collection sweep, while a general batch retry
would also repeat unrelated deferable outcomes. The separate operation verifies
exact checkpoint/inventory parity, internally selects only one non-redacted
timeout, makes one production-path attempt, and atomically replaces only that
private outcome. Repeated timeout and other systemic evidence remain blocking;
the boundary grants neither automatic retry loops nor collection-resume
authority.
