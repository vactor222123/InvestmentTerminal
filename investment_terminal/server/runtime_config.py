...
PROVIDER_MAX_OUTPUT_TOKENS_ENV = "INVESTMENT_TERMINAL_PROVIDER_MAX_OUTPUT_TOKENS"
PROVIDER_MAX_TOTAL_TOKENS_ENV = "INVESTMENT_TERMINAL_PROVIDER_MAX_TOTAL_TOKENS"
PROVIDER_MAX_TOTAL_COST_ENV = "INVESTMENT_TERMINAL_PROVIDER_MAX_TOTAL_COST"
PROVIDER_BUDGET_CURRENCY_ENV = "INVESTMENT_TERMINAL_PROVIDER_BUDGET_CURRENCY"
PROVIDER_INPUT_COST_PER_MILLION_TOKENS_ENV = "INVESTMENT_TERMINAL_PROVIDER_INPUT_COST_PER_MILLION_TOKENS"
PROVIDER_OUTPUT_COST_PER_MILLION_TOKENS_ENV = "INVESTMENT_TERMINAL_PROVIDER_OUTPUT_COST_PER_MILLION_TOKENS"
PROVIDER_PRICING_CURRENCY_ENV = "INVESTMENT_TERMINAL_PROVIDER_PRICING_CURRENCY"

RATE_LIMIT_CAPACITY_ENV = "INVESTMENT_TERMINAL_RATE_LIMIT_CAPACITY"
RATE_LIMIT_REFILL_PER_SECOND_ENV = "INVESTMENT_TERMINAL_RATE_LIMIT_REFILL_TOKENS_PER_SECOND"
...
class GroundedAIServerRuntimeConfig:
    ...
    def from_environment(cls, environment: Mapping[str, str]):
        ...
        server_api_key_environment_variable = environment.get(
            SERVER_API_KEY_ENV_NAME_ENV, DEFAULT_SERVER_API_KEY_ENV
        ).strip()
        if not server_api_key_environment_variable:
            raise ValueError(f"{SERVER_API_KEY_ENV_NAME_ENV} must not be empty")

        max_request_body_bytes = _positive_int(
            environment.get(MAX_REQUEST_BODY_BYTES_ENV, str(DEFAULT_MAX_REQUEST_BODY_BYTES)),
        ...
        provider_budget_currency = _currency_required(environment, PROVIDER_BUDGET_CURRENCY_ENV)
        provider_input_cost_per_million_tokens = _non_negative_decimal(
            _required(environment, PROVIDER_INPUT_COST_PER_MILLION_TOKENS_ENV),
            PROVIDER_INPUT_COST_PER_MILLION_TOKENS_ENV,
        )
        provider_output_cost_per_million_tokens = _non_negative_decimal(
            _required(environment, PROVIDER_OUTPUT_COST_PER_MILLION_TOKENS_ENV),
            PROVIDER_OUTPUT_COST_PER_MILLION_TOKENS_ENV,
        )
        provider_pricing_currency = _currency_required(environment, PROVIDER_PRICING_CURRENCY_ENV)
        if provider_budget_currency != provider_pricing_currency:
            raise ValueError(
                f"{PROVIDER_BUDGET_CURRENCY_ENV} must match {PROVIDER_PRICING_CURRENCY_ENV}"
            )

        rate_limit_capacity = _positive_int(
            environment.get(RATE_LIMIT_CAPACITY_ENV, str(DEFAULT_RATE_LIMIT_CAPACITY)),
            RATE_LIMIT_CAPACITY_ENV,
        )
        rate_limit_refill_tokens_per_second = _positive_decimal(
            environment.get(RATE_LIMIT_REFILL_PER_SECOND_ENV, str(DEFAULT_RATE_LIMIT_REFILL_PER_SECOND)),
    ...
    def governance_policy(self) -> GroundedProviderGovernancePolicy:
        return GroundedProviderGovernancePolicy(
            allowed_models=tuple(
                GroundedProviderModelAllowance(
                    provider_identity="OPENAI", model_identity=model
                )
...
def _currency_required(environment: Mapping[str, str], name: str) -> str:
