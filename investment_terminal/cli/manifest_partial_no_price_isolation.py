"""CLI for evidence-bound isolation of one reproduced no-price failure."""
...
def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(
        description="Isolate one checksum-bound reproduced no-price failure."
    )
    ...
    value.add_argument("--qualification", type=Path, required=True)
    value.add_argument("--qualification-checksum", required=True)
...
def main(
    ...
    *,
    ...
    try:
        ...
        updated, payload = ManifestPartialNoPriceIsolationService(
            clock=runtime_clock
        ).run(
            selection,
            checkpoint,
            qualification_bytes=options.qualification.read_bytes(),
            qualification_checksum=options.qualification_checksum,
        )
    except Exception:
        payload = _failed_payload(selection, runtime_clock(), "VALIDATION_OR_IO_ERROR")
    ...
    try:
        ...
        payload = _failed_payload(selection, runtime_clock(), "CHECKPOINT_WRITE_FAILED")
        report_writer(options.report_output, payload)
        if options.json:
            print(json.dumps(payload, indent=2, allow_nan=False))
        return 1
...
def _failed_payload(
    ...
    return {
        ...
        "failure": {
            "category": category,
            "reason": "Partial no-price isolation failed",
        },
        "limitations": [
