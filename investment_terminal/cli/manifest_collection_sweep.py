"""CLI for bounded manifest collection with deferred local candle defects."""
...
from investment_terminal.operations.manifest_collection_sweep import (
    ...
    validate_manifest_collection_checkpoints,
)
...
def main(argv=None, *, client=None, clock=None, writer=write_json_atomic) -> int:
    parser = argparse.ArgumentParser(
        description="Collect manifest batches while deferring local candle defects."
    )
    ...
    try:
        ...
        validate_manifest_collection_checkpoints(plan, read_checkpoint)
        ...
        payload = ManifestCollectionSweepService(
            importer=importer,
