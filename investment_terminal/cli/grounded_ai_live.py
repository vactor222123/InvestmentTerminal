...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--live",
        action="store_true",
        help="Explicitly permit the real provider/network call.",
    )
    parser.add_argument(
        "--database",
        type=Path,
    ...
    parser.add_argument(
        "--model",
        required=True,
        help="Exact OpenAI model identity to request.",
    )
    parser.add_argument(
        "--api-key-env",
        ...
        help=(
            ...
            "The secret value itself is never accepted as a CLI argument."
        ),
    )
    parser.add_argument(
        "--timeout-seconds",
...
def main(
    ...
    if not options.live:
        parser.error(
            "live OpenAI execution requires explicit --live"
        )
...
def _run_live(
    *,
    ...
    service = (
        ...
        else build_openai_grounded_generation_service(
...
def _print_human(
    ...
    print(
        f"Provider     : {trace['provider_identity']}"
    )
    print(
