...
that Nasdaq's yearly daily-market file is market-level and its per-symbol
volume statistics are Nasdaq-venue/reporting evidence rather than a neutral
consolidated-liquidity authority. The selected automatic path scans the complete
private universe through the existing Yahoo chart boundary using a fixed
...
Package 81 implements that boundary. It verifies the private projection
checksum, processes at most 100 symbols in deterministic order, accepts only a
unique exact-symbol three-letter currency, checkpoints every private outcome,
...
The controlled chart-metadata qualification succeeded: one item attempted and
one explicitly qualified, with matching evidence checksums. The provider path
therefore repairs the Search-field absence without currency inference. The next
...
Package 98 implements pure manifest selection plus one-batch execution. It
validates manifest identity/checksum, upstream evidence checksums, contiguous
indices, selected request schema/checksum, and only then composes persistence
...
Package 114 implements that complete vertical path. Manifest execution alone
uses the projected importer; checkpoint schema 2, resumable report schema 3,
and manifest/drain report schema 2 preserve explicit omission evidence while
...
Package 118 implements the shared provider-owned assessment and schema-2
...
Package 148 records the controlled sweep halt at batch 106. Seven items were
attempted and 7,543 candles inserted before a systemic `APIError`; no later
batch ran and SQLite integrity is `ok`. The current-run deferred counter
incorrectly counts that stopping failure, although ending deferred coverage
...
Package 150 implements a separate read-only production-path qualification for
the one `APIError` in the proper partial batch-106 checkpoint. It makes one
manifest-bound projection request, validates returned evidence, and reports
only aggregates or the existing privacy-safe causal category/type chain without
...
The Package 154 transition report is valid and checksum-bound. It records six
successes, one newly transitioned final `NO_PRICE_DATA` exclusion, zero
retryable failures, and 13 still-missing batch-106 members; report SHA-256 is
`6eb813eb379ab8f41712b5837a857fcaa633c28503559e4cc12ce44d911d510a`.
Package 155 authorizes one existing collection sweep with the exact remaining
...
Package 165 implements that reusable one-attempt boundary. It verifies strict
inventory bytes/checksum and exact current checkpoint parity before runtime
composition, selects only the unique recognized timeout, preserves every other
outcome and missing member, and atomically replaces only the selected schema-4
...
candles were downloaded and inserted. Ordered progress advanced from 41 to 66
completed batches, leaving 535. The existing batch-41 final
`RESPONSE_NUMERIC` exclusion remains explicit, and separately reported SQLite
...
out-of-order progress, and stops before the next batch on non-success.
...
Implement a separate read-only, manifest-bound one-series qualification with
strict projection and explicit repair provenance next. Do not change production
defaults, persist repaired candles, retry batch 19, or execute batch 20.
...
```text
develop @ 1ef0409dcb98f9877e2119578ecbdcb6ccd40eb4
```
