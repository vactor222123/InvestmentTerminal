...
def selection():
    ...
    return ManifestBatchSelection("a" * 64, 109, 601, request)


def checkpoint(selected, *, causal=True):
    return {
        "schema_version": 4,
        ...
        "outcomes": {
            ...
            "PRIVATE": {
                ...
                "causal_failure_evidence": (
                    {"category": "NO_PRICE_DATA", "exception_type_chain": CHAIN}
                    if causal else None
                ),
            },
        },
...
def diagnostic(selected, **changes):
    report = {
        "schema_version": 1,
...
def run(selected, value, evidence):
    return ManifestStoredNoPriceIsolationService(clock=lambda: NOW).run(
        selected,
        value,
        diagnostic_bytes=evidence[0],
        diagnostic_checksum=evidence[1],
    )


def test_transitions_only_bound_failure_and_preserves_missing_request_item():
    selected = selection()
    evidence = diagnostic(selected)
    ...
    assert final["isolation_policy_identity"] == "STORED_YAHOO_NO_PRICE_DATA_V1"
    assert final["isolation_evidence"] == {
        "causal_evidence_diagnostic_checksum": evidence[1]
    }
...
def test_rejects_diagnostic_checkpoint_causal_mismatch():
    selected = selection()
    evidence = diagnostic(selected, causal_failure_evidence={
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda selected, evidence: diagnostic(selected, batch_index=110), "binding"),
        (
            lambda selected, evidence: diagnostic(
                selected, status="LEGACY_EVIDENCE_UNAVAILABLE"
            ),
            "binding",
        ),
        (
...
def test_rejects_unbound_or_ineligible_evidence(mutation, match):
    selected = selection()
    evidence = mutation(selected, diagnostic(selected))

    with pytest.raises(ValueError, match=match):
        run(selected, checkpoint(selected), evidence)


def test_rejects_legacy_null_checkpoint_evidence():
    selected = selection()

    with pytest.raises(ValueError, match="does not match checkpoint"):
        run(selected, checkpoint(selected, causal=False), diagnostic(selected))
...
def test_rejects_conflicting_existing_final_evidence():
    ...
    updated["outcomes"]["PRIVATE"]["isolation_evidence"][
        "causal_evidence_diagnostic_checksum"
    ] = "b" * 64
...
def test_rejects_complete_checkpoint_before_transition():
    ...
    with pytest.raises(ValueError, match="proper partial"):
        run(selected, value, diagnostic(selected))


def test_rejects_duplicate_json_keys_before_transition():
    selected = selection()
    raw = diagnostic(selected)[0]
    ...
    with pytest.raises(ValueError, match="duplicate"):
        run(
            selected,
            checkpoint(selected),
            (duplicated, sha256(duplicated).hexdigest()),
        )
