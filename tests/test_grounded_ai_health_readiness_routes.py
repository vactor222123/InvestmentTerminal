...
from investment_terminal.server.runtime_config import (
    ALLOWED_MODELS_ENV, DATABASE_ENV, USAGE_COST_LEDGER_DATABASE_ENV, MODEL_ENV,
    ...
    PROVIDER_PRICING_CURRENCY_ENV, GroundedAIServerRuntimeConfig,
)
...
def handler():
    return GroundedAIHTTPHandler(application_service=SuccessService())


def readiness(database: Path, *, secret: str | None):
    ledger_database = database.with_name("provider_usage_cost.db")
    GroundedProviderUsageCostLedgerSQLiteStore(ledger_database).initialize()
    GroundedGenerationSQLiteStore(
        database.with_name("grounded_generations.db")
    ).initialize()
    ...
    if secret is not None:
        environment[DEFAULT_OPENAI_API_KEY_ENV] = secret
    return GroundedAIServerReadinessService(config=config, environment=environment)


def test_health_is_always_lightweight_200() -> None:
    client = TestClient(create_grounded_ai_fastapi_app(handler=handler()))
    response = client.get("/health")
    assert response.status_code == 200
    assert response.json() == {"status": "OK"}


def test_ready_returns_200_when_local_prerequisites_are_ready(tmp_path: Path) -> None:
    database = tmp_path / "knowledge.db"
    database.write_bytes(b"")
    client = TestClient(create_grounded_ai_fastapi_app(
        handler=handler(), readiness_service=readiness(database, secret="secret")
    ))
...
def test_ready_returns_503_when_credentials_are_missing(tmp_path: Path) -> None:
    database = tmp_path / "knowledge.db"
    database.write_bytes(b"")
    client = TestClient(create_grounded_ai_fastapi_app(
        handler=handler(), readiness_service=readiness(database, secret=None)
    ))
    response = client.get("/ready")
